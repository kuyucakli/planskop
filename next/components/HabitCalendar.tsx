...
export const HabitCalendar = ({ dailyPlan }: { dailyPlan: SelectActionPlan }) => {
    const { format_ical, get_human_readable_rrule, get_dt_str_from_rruleset } = useWasm() || {};
    ...
    const endDateStr = addAllowedDuration(dtstartObj, repeat).toISOString().slice(0, 10);
    const rrulesetStr = format_ical(dtstart, endDateStr);
    const dailyPlanDatesStr = get_dt_str_from_rruleset(rrulesetStr);
    const dailyPlanDatesArr = [...dailyPlanDatesStr.matchAll(/\d{4}-\d{2}-\d{2}/g)].map(m => new Date(m[0]));
            ...
            <ul>
                {dailyPlan.slots.map((s) => (
                    <li key={s.id} className="flex  rounded bg-gray-800 mt-0.5 ">
                        ...
                        <CldUploadButton uploadPreset="ml_default" className="bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 px-4  hover:border-blue-500 rounded flex-1/12">hi</CldUploadButton>
...
const ordinals: Record<string, string> = { "1.": "first", "2.": "second", "3.": "third", "4.": "fourth", "5.": "fifth" };